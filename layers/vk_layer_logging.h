...
static inline bool debug_log_msg(const debug_report_data *debug_data, VkFlags msg_flags, VkDebugReportObjectTypeEXT object_type,
    ...
    DebugReportObjectToAnnotObject(object_type, src_object, &object_name);
    ...
    if (0 != src_object) {
        ...
        if (VK_OBJECT_TYPE_QUEUE == object_name.objectType) {
            ...
            if (label_iter != debug_data->debugUtilsQueueLabels->end()) {
                ...
                if (nullptr != queue_labels) {
                    ...
                    uint32_t last_index = label_iter->second.size() - 1;
                    for (uint32_t label = 0; label < label_iter->second.size(); ++label) {
            ...
            if (label_iter != debug_data->debugUtilsCmdBufLabels->end()) {
                ...
                if (nullptr != cmd_buf_labels) {
                    ...
                    callback_data.cmdBufLabelCount = label_iter->second.size();
        ...
        if (utils_name_iter != debug_data->debugUtilsObjectNameMap->end()) {
            object_name = utils_name_iter->second.c_str();
        } else {
            auto marker_name_iter = debug_data->debugObjectNameMap->find(src_object);
            if (marker_name_iter != debug_data->debugObjectNameMap->end()) {
                object_name = marker_name_iter->second.c_str();
            }
        }
        if (object_name.size() > 0) {
            callback_data.pObjects[0].pObjectName = object_name.c_str();
            oss << " (Name = " << object_name << ")";
        } else {
            callback_data.pObjects[0].pObjectName = nullptr;
        }
    }
...
// Utility function to handle reporting of multiple objects
static inline bool debug_log_object_msg(const debug_report_data *debug_data,
                                        VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
                                        VkDebugUtilsMessageTypeFlagBitsEXT message_type, std::vector<VulkanObjectLogInfo> objects,
                                        int32_t msg_id_number, const char *msg_id_name, const char *message) {
    bool bail = false;
    VkLayerDbgFunctionNode *layer_dbg_node = NULL;
    ...
    DebugAnnotFlagsToReportFlags(message_severity, message_type, &debug_report_flags);
    VkDebugUtilsMessengerCallbackDataEXT callback_data;
    VkDebugReportObjectTypeEXT dr_object_type = VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT;
    uint64_t dr_object_handle = VK_NULL_HANDLE;
    uint32_t num_non_null_saved = 0;

    callback_data.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CALLBACK_DATA_EXT;
    callback_data.pNext = NULL;
    callback_data.flags = 0;
    callback_data.pMessageIdName = msg_id_name;
    callback_data.messageIdNumber = msg_id_number;
    callback_data.pMessage = message;
    callback_data.queueLabelCount = 0;
    callback_data.pQueueLabels = NULL;
    callback_data.cmdBufLabelCount = 0;
    callback_data.pCmdBufLabels = NULL;
    callback_data.objectCount = objects.size();

    VkDebugUtilsObjectNameInfoEXT *callback_objects = new VkDebugUtilsObjectNameInfoEXT[objects.size()];
    VkDebugUtilsLabelEXT *queue_labels = nullptr;
    VkDebugUtilsLabelEXT *cmd_buf_labels = nullptr;
    std::string new_debug_report_message = "";
    if (nullptr == callback_objects) {
        return false;
    }
    std::vector<std::string> object_names;
    object_names.resize(objects.size());
    for (uint32_t obj = 0; obj < objects.size(); ++obj) {
        std::ostringstream oss;
        callback_objects[obj].sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_OBJECT_NAME_INFO_EXT;
        callback_objects[obj].pNext = nullptr;
        callback_objects[obj].objectType = get_object_type_enum[objects[obj].type];
        callback_objects[obj].objectHandle = objects[obj].handle;

        if (0 != objects[obj].handle) {
            // If this is a queue, add any queue labels to the callback data.
            if (kVulkanObjectTypeQueue == objects[obj].type) {
                auto label_iter = debug_data->debugUtilsQueueLabels->find(reinterpret_cast<VkQueue>(objects[obj].handle));
                if (label_iter != debug_data->debugUtilsQueueLabels->end()) {
                    assert(queue_labels == nullptr);
                    queue_labels = new VkDebugUtilsLabelEXT[label_iter->second.size()];
                    if (nullptr != queue_labels) {
                        // Record the labels, but record them in reverse order since we want the
                        // most recent at the top.
                ...
                // If this is a command buffer, add any command buffer labels to the callback data.
            } else if (kVulkanObjectTypeCommandBuffer == objects[obj].type) {
                auto label_iter = debug_data->debugUtilsCmdBufLabels->find(reinterpret_cast<VkCommandBuffer>(objects[obj].handle));
                if (label_iter != debug_data->debugUtilsCmdBufLabels->end()) {
                    assert(cmd_buf_labels == nullptr);
                    cmd_buf_labels = new VkDebugUtilsLabelEXT[label_iter->second.size()];
                    if (nullptr != cmd_buf_labels) {
                        // Record the labels, but record them in reverse order since we want the
                        // most recent at the top.
            ...
            // Look for any debug utils or marker names to use for this object
            object_names[obj] = "";
            auto utils_name_iter = debug_data->debugUtilsObjectNameMap->find(objects[obj].handle);
            if (utils_name_iter != debug_data->debugUtilsObjectNameMap->end()) {
                object_names[obj] = utils_name_iter->second.c_str();
            } else {
                auto marker_name_iter = debug_data->debugObjectNameMap->find(objects[obj].handle);
                if (marker_name_iter != debug_data->debugObjectNameMap->end()) {
                    object_names[obj] = marker_name_iter->second.c_str();
                }
            }
            if (object_names[obj].size() > 0) {
                callback_objects[obj].pObjectName = object_names[obj].c_str();
            } else {
                callback_objects[obj].pObjectName = nullptr;
            }

            // For VK_EXT_debug_report, the first two non-VK_NULL_HANDLE are the most important things to write out.
            if (num_non_null_saved < 2) {
                std::string object_type_string = object_string[objects[obj].type];
                if (num_non_null_saved > 0) {
                    oss << ", ";
                } else {
                    dr_object_type = get_debug_report_enum[objects[obj].type];
                    dr_object_handle = objects[obj].handle;
                }
                oss << "Object[" << num_non_null_saved << "]: " << object_type_string << " 0x" << std::hex << objects[obj].handle;
                if (object_names[obj].size() > 0) {
                    callback_objects[obj].pObjectName = object_names[obj].c_str();
                    oss << " (Name = " << object_names[obj] << ")";
                }
                new_debug_report_message += oss.str();
                num_non_null_saved++;
            }
    ...
    new_debug_report_message += " | ";
    new_debug_report_message += message;
    callback_data.pObjects = callback_objects;

    while (layer_dbg_node) {
        if (!layer_dbg_node->is_messenger && layer_dbg_node->report.msgFlags & debug_report_flags &&
            layer_dbg_node->report.pfnMsgCallback(debug_report_flags, dr_object_type, dr_object_handle, 0, msg_id_number,
                                                  msg_id_name, new_debug_report_message.c_str(), layer_dbg_node->pUserData)) {
            bail = true;
        } else if (layer_dbg_node->is_messenger && (layer_dbg_node->messenger.messageSeverity & message_severity) &&
                   (layer_dbg_node->messenger.messageType & message_type) &&
                   layer_dbg_node->messenger.pfnUserCallback(message_severity, message_type, &callback_data,
...
#ifndef WIN32
static inline bool log_object_msg(const debug_report_data *debug_data, VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
                                  VkDebugUtilsMessageTypeFlagBitsEXT message_type, std::vector<VulkanObjectLogInfo> objects,
                                  int32_t msg_id_number, const char *msg_id_name, const char *format, ...)
    __attribute__((format(printf, 7, 8)));
#endif
static inline bool log_object_msg(const debug_report_data *debug_data, VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
                                  VkDebugUtilsMessageTypeFlagBitsEXT message_type, std::vector<VulkanObjectLogInfo> objects,
                                  int32_t msg_id_number, const char *msg_id_name, const char *format, ...) {
    if (!debug_data || !(debug_data->active_severities & message_severity) || !(debug_data->active_types & message_type)) {
        // Message is not wanted
        return false;
    }
    ...
    va_end(argptr);
    bool result = debug_log_object_msg(debug_data, message_severity, message_type, objects, msg_id_number, msg_id_name,
                                       str ? str : "Allocation failure");
    free(str);
    return result;
}
...
static inline VKAPI_ATTR VkBool32 VKAPI_CALL report_log_callback(VkFlags msg_flags, VkDebugReportObjectTypeEXT obj_type,
    ...
    print_msg_flags(msg_flags, msg_flag_string);
...
static inline VKAPI_ATTR VkBool32 VKAPI_CALL messenger_log_callback(VkDebugUtilsMessageSeverityFlagBitsEXT message_severity,
    ...
    print_msg_severity(message_severity, msg_severity);
    print_msg_type(message_type, msg_type);
